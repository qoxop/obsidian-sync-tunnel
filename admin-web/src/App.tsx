import { AuditOutlined, DatabaseOutlined, DashboardOutlined, FileTextOutlined, LogoutOutlined, MenuFoldOutlined, MenuUnfoldOutlined, SafetyCertificateOutlined, ToolOutlined } from "@ant-design/icons";
...
type Page = "dashboard" | "vaults" | "audit" | "logs" | "maintenance";
...
export default function App() {
  ...
  const content = page === "vaults" ? <VaultsPage api={api} /> : page === "audit" ? <AuditPage api={api} /> : page === "logs" ? <LogsPage api={api} /> : page === "maintenance" ? <MaintenancePage api={api} /> : <DashboardPage api={api} />;
    ...
    <Layout className="app-layout">
      <Layout.Sider collapsible collapsed={collapsed} trigger={null} width={240} theme="light" className="app-sider">
        ...
        <Menu mode="inline" selectedKeys={[page]} onClick={({ key }) => setPage(key as Page)} items={[
          { key: "dashboard", icon: <DashboardOutlined />, label: "概览" }, { key: "vaults", icon: <DatabaseOutlined />, label: "Vault 与设备" },
		  { key: "audit", icon: <AuditOutlined />, label: "审计日志" }, { key: "logs", icon: <FileTextOutlined />, label: "运行日志" },

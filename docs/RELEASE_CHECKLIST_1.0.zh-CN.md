...
本清单区分“仓库代码准备”和“需要仓库管理员在 GitHub/设备上完成的发布”。任何 Tag/Release/GHCR 推送都是外部写操作，必须由仓库管理员明确执行。
...
- [ ] Settings > Actions > General 允许 Actions 使用 `GITHUB_TOKEN` 创建 Release 和写 GHCR；
...
- 构建 `linux/amd64`、`linux/arm64` GHCR 镜像并附 provenance/SBOM；
- 使用 GitHub OIDC + Cosign keyless 签名镜像 digest；
- RC 不更新 `latest` 镜像标签。
...
- [ ] `docker pull ghcr.io/qoxop/obsidian-sync-tunnel:1.0.0-rc.1` 成功；
- [ ] `docker buildx imagetools inspect` 显示 amd64/arm64；
- [ ] GitHub Attestations 可验证 provenance；
- [ ] `cosign verify` 使用 GitHub Actions issuer/仓库 identity 验证 digest；
- [ ] 从 Release 安装的插件而不是本地构建产物执行完整人工验收；
- [ ] GHCR 镜像而不是本地镜像执行备份恢复演练。
...
正式版重新运行版本脚本为 `1.0.0`，提交后打同名 annotated Tag。工作流会创建 Stable Release，并同时发布 GHCR `1.0.0` 与 `latest`。绝不移动 `1.0.0-rc.1` Tag，也不在失败工作流上手工拼接不一致附件。

## F. 发布后

- [ ] 新设备严格通过一次性配对加入；
- [ ] 监控 GitHub Issues/Private Vulnerability Reporting，但不收集客户端遥测；
- [ ] 定期 Dependabot、CodeQL、Nightly 和备份恢复演练；

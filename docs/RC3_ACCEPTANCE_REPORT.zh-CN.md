...
- 两个 Windows 测试 Vault 已通过 BRAT 更新并固定到 `1.0.0-rc.4`，插件完成热重载；两端 cursor 与 acknowledged revision 均为 265，各跟踪 128 个文件，全部队列和同步计数为 0；
- macOS 实机此前已完成双向同步、中断续传、重命名、批量删除以及客户端和服务端重启恢复；
- 至少一台移动真机已完成功能验收，跨设备同步未发现功能问题；验收中发现排除路径编辑框宽度未对齐，已在稳定版中修复；
- 包含移动端布局修复的提交已通过 GitHub CI 与 CodeQL，本地插件类型检查、34 项测试和生产构建均通过。
